...
public class JavaSimpleEditor extends JFrame {
    ...
    private void init() {
        ...
        // Create Load from Server button
        JButton loadButton = new JButton("Load from Server");
        loadButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                new Thread(new LoadFromServerTask()).start();
            }
        });
        ...
        public void run() {
            try (Socket socket = new Socket("localhost", 8808);
                ...
                if (fileName != null && !fileName.trim().isEmpty()) {
                    ...
                    String response = fileContent.toString();
                    if (!"File not found".equals(response.trim())) {
                ...
                ex.printStackTrace();
                JOptionPane.showMessageDialog(null, "Error loading from server");
            }
        }
    }
